...
const WaitingArea = (props: WaitingAreaProps) => {
	...
	return (
		<div className='w-80 px-10 pt-5 flex-col justify-start items-center gap-3 inline-flex bg-PrimaryFocus'>
			...
			<div className='text-center text-black text-base font-normal leading-snug'>{direction}</div>
					...
					return (
						<div className={`justify-start items-start gap-6 inline-flex ${selected ? 'text-AlertMain' : ''}`} key={i}>
							<div className={`justify-start items-start gap-1 flex`}>
								<div className='text-center text-base font-normal leading-snug'>{StartTime.getHours()}:{StartTime.getMinutes()}</div>
								<div className='text-center text-lg font-normal leading-snug'>→</div>
								<div className='text-center text-base font-normal leading-snug'>{EndTime.getHours()}:{EndTime.getMinutes()}</div>
							</div>
